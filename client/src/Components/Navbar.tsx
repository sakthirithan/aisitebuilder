    ...
    <>
      <nav className='z-50 flex items-center justify-between w-full py-4 px-4 md:px-16 lg:px-24 xl:px-32 backdrop-blur border-b text-white border-slate-800'>
        ...
        <div className='flex items-center gap-3'>
          <button onClick={() => navigate('/auth/signin')} className='px-6 py-1.5 max-sm:text-sm bg-indigo-600 active:scale-95 hover:bg-indigo-700 rounded-lg'>
            Get Started
          </button>
      ...
      {MenuOpen && (
        <div className='fixed inset-0 z-100 bg-black/60 text-white backdrop-blur flex flex-col items-center justify-center text-lg gap-8 md:hidden transition-transform duration-300'>
          ...
          <button className='active:ring-3 active:ring-white aspect-square size-10 p-1 flex items-center justify-center bg-slate-100 hover:bg-slate-200 text-black rounded-md' onClick={() => setMenuOpen(false)}>
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg>
          </button>
        </div>
      )}

      {/* Backdrop Blue theme image */}
